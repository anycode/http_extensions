...
class ProtobufExtension extends Extension<ProtobufOptions> {
  ...
  ProtobufExtension({ProtobufOptions defaultOptions = const ProtobufOptions(), this.logger})
      : assert(defaultOptions.requestMessage == null, 'Global protobuf options should have an empty request message'),
        assert(defaultOptions.responseMessage == null, 'Global protobuf options should have an empty response message'),
  ...
  BaseRequest _createRequest(BaseRequest original, ProtobufOptions options) {
    ...
    final request = ProtobufRequest.fromRequest(original, options.requestMessage!);
  ...
  Future<StreamedResponse> sendWithOptions(
    ...
    if (options.requestMessage != null && options.shouldSerialize(request)) {
      logger?.fine('[${request.url}] Serializing ${options.requestMessage.runtimeType} body with protobuf');
    ...
    var result = await super.sendWithOptions(request, options, cancellationToken: cancellationToken);

    if (options.responseMessage != null && options.shouldDeserialize(result)) {
      logger?.fine('[${request.url}] Deserializing ${options.responseMessage.runtimeType} content with protobuf');
      final responseBytes = await result.stream.toBytes();
      options.responseMessage!.mergeFromBuffer(responseBytes);
      return StreamedResponse(Stream.fromIterable([responseBytes]), result.statusCode,
