...
class CacheExtension extends Extension<CacheOptions> {
  ...
  Future<StreamedResponse> sendWithOptions(
    ...
    var shouldUpdate = options.forceUpdate || cacheResult == null || (!options.forceCache && cacheResult.expiry.isBefore(DateTime.now()));

    if (shouldUpdate) {
      logger?.fine('[$cacheId][${request.url}] Not existing or expired cache, or forced update : starting a new request');

      try {
        if (cacheResult != null && !request.headers.containsKey(HttpHeaders.ifModifiedSinceHeader)) {
          logger?.fine('[$cacheId][${request.url}] Adding `${HttpHeaders.ifModifiedSinceHeader}` header');
          request.headers[HttpHeaders.ifModifiedSinceHeader] = HttpDate.format(cacheResult.downloadedAt);
        }

        final response = await super.sendWithOptions(request, options, cancellationToken: cancellationToken);

        if (cacheResult != null && response.statusCode == 304) {
          logger?.fine('[$cacheId][${request.url}] Content not modified (status code 304), returning cache');
          return cacheResult;
        }

        final expiry = DateTime.now().add(options.expiry);
        cacheResult = await CachedResponse.fromResponse(response, expiry: expiry, id: cacheId, request: request);

        if (options.shouldBeSaved(cacheResult)) {
          logger?.fine('[$cacheId][${request.url}] Saving resulting response to cache ...');
